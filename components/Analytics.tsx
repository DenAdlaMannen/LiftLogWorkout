...
const Analytics: React.FC<AnalyticsProps> = ({ history, bodyWeightHistory, onAddWeight, onBack }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Trend calculation for Volume: Compare last 3 sessions to the 3 sessions before that
    let trend: 'up' | 'down' | 'neutral' = 'neutral';
    if (history.length >= 2) {
      const sortedVolumes = [...history]
        .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
        .map(s => s.exercises.reduce((acc, ex) => acc + (ex.weight * ex.reps * ex.sets), 0));
          ...
          <div className="flex items-center justify-between">
            ...
            <form onSubmit={handleWeightSubmit} className="flex gap-2">
              <input 
                ...
                placeholder="0.0" className="w-20 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-sm focus:ring-1 focus:ring-emerald-500 outline-none" 
              />
